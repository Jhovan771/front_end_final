...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      Axios.get(`${server_url}/api/getUserId`, {
  ...
  const deleteSection = async (sectionName) => {
    try {
      const response = await Axios.delete(
        `${server_url}/api/deleteSection/${sectionName}`
      );
      ...
      console.error(error);
      alert(`An error Occured in deleting section. \n Make sure the section
        list is empty.`);
    } finally {
      setTimeout(() => {
  ...
  const updateList = async () => {
    try {
      const response = await Axios.put(`${server_url}/api/updateSName`, {
        newSName: sname,
        sname: originalSName,
      });
  ...
  const addlist = async () => {
    try {
      const response = await Axios.post(`${server_url}/api/insection`, {
        sname: sname,
  ...
  useEffect(() => {
    const fetchSectionData = async () => {
      try {
        ...
        const response = await Axios.get(`${server_url}/api/sectionList`, {
          headers: {
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const response = await Axios.post(`${server_url}/api/students`, {
        firstName: firstName,
        lastName: lastName,
        sectionID: sectionID,
      });
          ...
          <table className='section-list'>
            <thead>
              <tr>
                ...
                <th className='th-control'>MENU</th>
              </tr>
            </thead>
            <tbody>
